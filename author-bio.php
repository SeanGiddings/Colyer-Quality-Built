...
<div class="author-info<?php if ( ! is_single() ) : ?> bg-faded<?php endif; ?>">
	<div class="row">
		<div class="col-sm-12">
			<h2>
				<?php
					echo get_avatar( get_the_author_meta( 'user_email' ), apply_filters( 'colyer_wp_author_bio_avatar_size', 48 ) ) . '&nbsp;';
					printf( esc_html__( 'About %s', 'colyer-wp' ), get_the_author() );
				?>
			</h2>
			...
			<div class="author-links">
				<?php
					if ( ! empty( get_the_author_meta( 'user_url' ) ) ) :
						printf( '<a href="%s" class="www btn btn-secondary btn-sm">' . esc_html__( 'Website', 'colyer-wp' ) . '</a>', esc_url( get_the_author_meta( 'user_url' ) ) );
					endif;
